defmodule DeployEx.MimirBlockLegibilityTest do
  ...
  # independent agents in a consumer repo deleted the grafana_mimir_url
  # metrics block from their RENDERED alloy_config.alloy.j2 as dead code — one
  # landed. grafana_mimir_url is undefined in that repo, so the {% if %} never
  # fires there; a reader with only that repo in view sees a guard on an
  # undefined variable, directly below the closing `}` of loki.write, with
  # nothing explaining it.
  #
  # Upstream (origin/main) the block is well protected — mimir_role_test.exs
  # and priv_renderer_mimir_test.exs both pin it, so deleting it there fails
  # tests immediately. Nothing protects the RENDERED copy a consumer actually
  # edits, and a per-consumer comment is the wrong fix (ansible.build's
  # unguarded File.cp_r! overwrites any local template edit on every
  # regeneration, but a consumer can still delete the block from the deployed
  # artifact before the next regen runs). The explanation has to live where it
  # reaches the reader who deletes it: the rendered artifact itself.
  #
  # ROLE-LEVEL SCOPE (MEASURED against origin/main by the brief's author, not
  # re-derived here): the consumer's copy of grafana_alloy/tasks/main.yaml has
  # drifted 47 lines against deploy_ex's origin/main copy, while the template
  # has drifted only 29 — the task file is where local engineers spend
  # edit/read attention, the template is the file they touch least. A guard
  # living only in the low-drift template is invisible to anyone auditing the
  # role primarily through its (higher-drift, more-read) task list. So the
  # explanation is added in BOTH places: a `//` comment in the template (what
  # ships to production and what the three deleting agents were literally
  # looking at), and a `#` comment in tasks/main.yaml next to the task that
  # renders it (what a role-level auditor reads first). Same fix ported from
  # origin/main's fix/mimir-block-legibility onto this tree's differently-
  # reconciled tasks/main.yaml (opgg's own unzip-availability fix lives in the
  # same file — this comment is inserted independently of that content).
  ...
  # The immediate parent commit on this branch — grafana_mimir_url_configured
  # predicate fix, no legibility comment yet. Resolved by SHA, not a floating
  # ref, so this stays the correct "before" snapshot regardless of what HEAD
  # becomes once this change is committed.
  @before_commit_sha "7049e28bc373333e608719b13e7650bd4bdec42d"
  ...
  describe "alloy_config.alloy.j2 — legibility comment" do
    ...
    test "the comment survives rendering into the output — F59 scoped-in arm" do
      context =
        @base_context
        |> Map.put("grafana_mimir_url", "http://10.0.1.40:8080")
        |> Map.put("grafana_mimir_url_configured", true)
        |> Map.put("app_name", "pipeline")
  ...
  describe "grafana_alloy/tasks/main.yaml — role-level legibility comment" do
    ...
    test "opgg's unrelated unzip-availability comment is untouched" do
      content = File.read!(@tasks_path)

      assert content =~ "Ubuntu's"
      assert content =~ "MEASURED on Ubuntu"
    end
