defmodule DeployEx.GrafanaAlloyRoleTest do
  ...
  #
  # PORTED VERBATIM from origin/main @ 6dedca4 (commit 975c5d3, "fix(grafana_alloy):
  # version-aware download and a journal precondition") onto opgg/main as part of the
  # mimir-core-port sprint. This file is self-contained (parses the role's own YAML,
  # no dependency on DeployEx.Mimir/priv_renderer/terraform), so it ported with zero
  # content changes — only the role file it exercises (tasks/main.yaml) was 3-way
  # reconciled: opgg's guarded `Install unzip` fix + this file's journal-precondition
  # and version-aware-marker fixes + this port's readiness-wait/handler/promtail-loop
  # content. See mimir-core-port.md report for the reconciliation detail.
  #
