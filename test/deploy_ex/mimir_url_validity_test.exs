defmodule DeployEx.MimirUrlValidityTest do
  ...
  describe "group_vars/all.yaml.eex — grafana_mimir_url_configured (the one definition)" do
    ...
    test "is declared in the SAME is_mimir_enabled scope as grafana_mimir_url itself — never one without the other" do
      content = File.read!(@group_vars_path)

      # This tree DOES have an is_mimir_enabled/FILL_IN_AFTER_FIRST_APPLY sentinel
      # convention already (unlike the port's original base, which had neither) —
      # loki/prometheus/sentry/mimir all follow it. grafana_mimir_url_configured
      # is scoped alongside grafana_mimir_url within that SAME conditional block,
      # not declared separately outside it: `--no-mimir` must omit both together
      # (DeployEx.PrivRendererMimirTest's byte-identical --no-mimir baseline pins
      # this), and a consumer with Mimir enabled always gets both.
      [is_mimir_enabled_block] = Regex.run(~r/<%= if @is_mimir_enabled do %>.*?<% end %>/s, content)

      assert is_mimir_enabled_block =~ "grafana_mimir_url_configured"
      assert is_mimir_enabled_block =~ "grafana_mimir_url:"
    end
  end
